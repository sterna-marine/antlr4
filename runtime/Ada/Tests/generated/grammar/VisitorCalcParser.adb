...
-- open
type VisitorCalcParser is new Parser with null record;

   -- internal static
   _decisionToDFA : DFA_List = {
          decisionToDFA := DFA_Container.Empty_Vector;
          length : constant := VisitorCalcParser._ATN.getNumberOfDecisions();
          for i in 0..<length {
   ...
   type SContext is new ParserRuleContext with null record;
         ...
         function EOF (This : …) return Optional_TerminalNode
            is (getToken(VisitorCalcParser.Tokens.EOF.rawValue, 0));
      -- open
      overriding
      function getRuleIndex (This : …) return Integer
         is (VisitorCalcParser.RULE_s);
      -- open
      overriding
      procedure enterRule (This : …; listener : ParseTreeListener) is
      begin
         if listener : constant := listener Optional_as VisitorCalcListener then
      ...
      function accept (This : …; visitor : ParseTreeVisitor<T>) return Optional_T {
         if visitor : constant := visitor Optional_as VisitorCalcVisitor then
             return visitor.visitS(self);
         elsif visitor : constant := visitor Optional_as VisitorCalcBaseVisitor then
             return visitor.visitS(self);
         else
   ...
   -- with No_Return;
    ...
    begin
      ...
      enterRule(_localctx, 0, VisitorCalcParser.RULE_s); -- try
      defer :
         begin
         end defer;
             exitRule(); -- try!
      ...
      do :
         ...
         exception
          ...
          expr(0); -- try
          setState(5)
          match(VisitorCalcParser.Tokens.EOF.rawValue); -- try
   ...
   type ExprContext is new ParserRuleContext with null record;
      ...
      overriding
      function getRuleIndex (This : …) return Integer
         is (VisitorCalcParser.RULE_expr);
   end if;
   -- public
   type AddContext is new ExprContext with null record;
         ...
         function ADD (This : …) return Optional_TerminalNode
            is (getToken(VisitorCalcParser.Tokens.ADD.rawValue, 0));
         -- open
         function SUB (This : …) return Optional_TerminalNode
            is (getToken(VisitorCalcParser.Tokens.SUB.rawValue, 0));
   ...
   type NumberContext is new ExprContext with null record;
         ...
         function INT (This : …) return Optional_TerminalNode
            is (getToken(VisitorCalcParser.Tokens.INT.rawValue, 0));
   ...
   type MultiplyContext is new ExprContext with null record;
         ...
         function MUL (This : …) return Optional_TerminalNode
            is (getToken(VisitorCalcParser.Tokens.MUL.rawValue, 0));
         -- open
         function DIV (This : …) return Optional_TerminalNode
            is (getToken(VisitorCalcParser.Tokens.DIV.rawValue, 0));
   ...
   begin
      ...
      enterRecursionRule(_localctx, 2, VisitorCalcParser.RULE_expr, _p); -- try
      var _la : Integer := 0
      defer :
         begin
         end defer;
      ...
      do :
         ...
         setState(8)
         match(VisitorCalcParser.Tokens.INT.rawValue); -- try
         ...
         while (_alt != 2 and then _alt != ATN.INVALID_ALT_NUMBER) {
            if ( _alt = 1 ) {
               ...
               case 1 :
                  ...
                  pushNewRecursionContext(_localctx, _startState, VisitorCalcParser.RULE_expr); -- try
                  setState(10)
                  if (!(precpred(_ctx, 2))) {
                  ...
                  setState(11)
                  _la := try _input.LA(1)
                  if (!(_la  =  VisitorCalcParser.Tokens.MUL.rawValue or else _la  =  VisitorCalcParser.Tokens.DIV.rawValue)) {
               ...
               case 2 :
                  ...
                  setState(14)
                  _la := try _input.LA(1)
                  if (!(_la  =  VisitorCalcParser.Tokens.ADD.rawValue or else _la  =  VisitorCalcParser.Tokens.SUB.rawValue)) {
                  _errHandler.recoverInline(self); -- try
                  else
                     _errHandler.reportMatch(self)
                     consume(); -- try
