...
type VisitorTests is new XCTestCase with null record;
{
...
begin
        ...
        parser : constant := try VisitorBasicParser(CommonTokenStream(lexer))

        context : constant := try parser.s()
...
{
            override procedure Init (This : …) is
begin
                super.init()
            end ;
...
begin
        ...
        parser : constant := try VisitorCalcParser(CommonTokenStream(lexer))
