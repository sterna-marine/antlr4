...
package body ANTLR.Runtime.ATN.ParserATNSimulator is
   ...
   begin
      ...
      --   DOTGenerator dot := new DOTGenerator ( (Valid => False));
   ...
   begin
      for d in 0 .. decisionToDFA - 1.count loop
            decisionToDFA.Insert (Key => d, New_Item => DFA (atn.getDecisionState (d)!, d));
      end loop;
   end clearDFA;
...
begin
            if debug or else trace_atn_sim then
               Text_IO.Put_Line ("execATN decision " & dfa.decision & " exec LA (1)==" & getLookaheadName (input) & " line " & input.LT (1)!.getLine () & ":" & input.LT (1)!.getCharPositionInLine ());
            end if;

            previousD := s0

            if debug then
               Text_IO.Put_Line ("s0 := " & s0'Image);
            end if;
            ...
            loop
               ...
               if D.requiresFullContext and then (mode /= PredictionModes.SLL) then
                  ...
                  if preds : constant := D.predicates then
                        if debug then
                           Text_IO.Put_Line ("DFA state has preds in DFA sim LL failover");
                        end if;
                        ...
                        if conflictingAlts.cardinality () == 1 then
                           if debug then
                              Text_IO.Put_Line ("Full LL avoided");
                           end if;
                  ...
                  if dfa_debug then
                        Text_IO.Put_Line ("ctx sensitive state " & outerContext'Image & " in " & D'Image);
                  end if;
   ...
   begin
      ...
      if edges =  (Valid => False) or else (t + 1) < 0 or else (t + 1) >= (edges!.count) then
            return  (Valid => False);
      end if;
...
begin
      ...
      if debug then
            ...
            Text_IO.Put_Line ("SLL altSubSets=" & altSubSets'Image & ", configs=" & reach'Image & ", predict=" & predictedAlt'Image & ", allSubsetsConflict=" & PredictionModes.allSubsetsConflict (altSubSets) & ", conflictingAlts=" & getConflictingAlts (reach));
      end if;
      ...
      if D.isAcceptState and then D.configs.hasSemanticContext then
            ...
            if D.predicates /=  (Valid => False) then
...
begin
      if debug or else trace_atn_sim then
            Text_IO.Put_Line ("execATNWithFullContext " & s0'Image);
      end if;
      fullCtx : constant := True;
      ...
      loop
            ...
            if reach : constant := reach then
               altSubSets : constant PredictionMode := PredictionModes.getConflictingAltSubsets (reach);
               if debug then
                  Text_IO.Put_Line ("LL altSubSets=" & altSubSets'Image & ", predict=" & PredictionModes.getUniqueAlt (altSubSets) & ", resolvesToJustOneViableAlt=" & PredictionModes.resolvesToJustOneViableAlt (altSubSets));
               end if;
   ...
   begin
      if debug then
            Text_IO.Put_Line ("in computeReachSet, starting closure: " & closureConfigSet'Image);
      end if;

      if DoubleKeyMap.Is_Empty (mergeCache) then --TOFIX
            This.mergeCache := DoubleKeyMap.Empty_Map; -- PredictionContext.DoubleKeyMap ();
      end if;
      ...
      for config in configs loop
            if debug then
               Text_IO.Put_Line ("testing " & getTokenName (t) & " at " & Image (config));
            end if;
            ...
            if config.state is RuleStopState then
               ...
               if fullCtx or else t = BufferedTokenStream.EOF then
                  if skippedStopStates =  (Valid => False) then
      ...
      --
      if skippedStopStates =  (Valid => False) and then t /= CommonToken.EOF then
      ...
      if reach =  (Valid => False) then
            ...
            for config in intermediate.configs loop
               closure (config, reach!, closureBusy'Access, False, fullCtx, treatEofAsEpsilon);
            end loop;
      end if;
...
begin
   ...
   function getPredsForAmbigAlts (This : ParserATNSimulator; ambigAlts : BitSet;
      ...
      nalts : Integer) return SemanticContext.Container.Vector is
            ...
            if debug then
               Text_IO.Put_Line ("getPredsForAmbigAlts result " & UString (describing => altToPred));
            end if;
            return altToPred
   end if;
...
begin
            ...
            for pair in predPredictions loop
               ...
               if debug or else dfa_debug then
                  Text_IO.Put_Line ("eval pred " & pair'Image & "= " & predicateEvaluationResult'Image);
               end if;

               if predicateEvaluationResult then
                  if debug or else dfa_debug then
                        Text_IO.Put_Line ("PREDICT " & pair.alt);
                  end if;
                  predictions.set (pair.alt); -- try!
                  exit when not complete;
               end if;
...
begin
   ...
   procedure closure (config : ATNConfig;
      ...
      treatEofAsEpsilon  : Boolean) {
            ...
            closureCheckingStopState (config, configs, closureBusy'Access, collectPredicates, fullCtx, initialDepth, treatEofAsEpsilon);
   ...
   procedure closureCheckingStopState (config : ATNConfig;
      ...
      treatEofAsEpsilon  : Boolean) {
            ...
            if debug then
               Text_IO.Put_Line ("closure (" + config.toString (parser, True) + ")");
            end if;
                  ...
                  for i in 0 .. length - 1 loop
                        if configContext.getReturnState (i) == PredictionContext.EMPTY_RETURN_STATE then
                           if fullCtx then
                              ...
                              if debug then
                                    Text_IO.Put_Line ("FALLING off rule" & getRuleName (config.state.ruleIndex!));
                              end if;
                              closure_ (config, configs, closureBusy'Access, collectPredicates,;
            ...
            closure_ (config, configs, closureBusy'Access, collectPredicates, fullCtx, depth, treatEofAsEpsilon);
   }
            ...
            for i in 0 .. length - 1 loop
               ...
               if c : constant := c then
                  ...
                  if config.state is RuleStopState then
                        ...
                        if debug then
                           Text_IO.Put_Line ("dips into outer ctx: " & c'Image);
                        end if;
                  else
...
begin
      ...
      for  i in 0 ..< numCtxs loop -- for each stack context
            ...
            returnState : constant := atn.states.Element (returnStateNumber)!
...
begin
      if debug then
            Text_IO.Put_Line ("ACTION edge " & t.ruleIndex & ":" & t.actionIndex);
      end if;
      return ATNConfig (config, t.target);
   end if;
   ...
   begin
      if debug then
            Text_IO.Put_Line ("PRED (collectPredicates=" & collectPredicates'Image & ") " & pt.precedence & ">=_p, ctx dependent=True");
            --if ( parser /=  (Valid => False) )  then
            Text_IO.Put_Line ("context surrounding pred is " & parser.getRuleInvocationStack ());
            -- }
      end if;

      c : Optional_ATNConfig; :=  (Valid => False);
      ...
      if debug then
            Text_IO.Put_Line ("config from pred transition=" & c?.description, Default => (Valid => False) & ")"); --TOFIX
      end if;
      return c
   end if;
   ...
   begin
      if debug then
            Text_IO.Put_Line ("PRED (collectPredicates=" & collectPredicates'Image & ") " & pt.ruleIndex & ":" & pt.predIndex & ", ctx dependent=" & pt.isCtxDependent);
            -- if parser /=  (Valid => False) then
            Text_IO.Put_Line ("context surrounding pred is " & parser.getRuleInvocationStack ());
            -- end if;
      end if;
...
begin
      if debug then
            Text_IO.Put_Line ("CALL rule " & getRuleName (t.target.ruleIndex!) & ", ctx=" & config.context?.description, Default => (Valid => False) & ")");
      end if;
...
begin
   ...
   -- public final
   function getLookaheadName (This : ParserATNSimulator;input : TokenStream) return UString is
begin
   ...
   begin
      Text_IO.Put_Line (Standard_Error, "dead end configs: ");
      for c in nvae.getDeadEndConfigs ()!.configs loop
            ...
            if c.state.getNumberOfTransitions () > 0 then
               ...
               if Is_Valid (at) then
                  ...
                  if Is_Valid (st) then
                  not_sign : constant Bolean := st is NotSetTransition
                  if not_sign then
                     trans := "~"  & "Set " & st.set.description;
                  else
                     trans := "" & "Set " & st.set.description;
               end if;
            end if;
            Text_IO.Put_Line (Standard_Error, "" & c.toString (parser, True) & ":" & trans'Image);
      end if;
   end if;
...
begin
      startToken : constant := input.get (startIndex); -- try!
...
begin
   ...
   begin
      to := to
      if debug then
            Text_IO.Put_Line ("EDGE " & from'Image & " -> " & to'Image & " upon " & getTokenName (t));
      end if;
   ...
   function addDFAState (This : ParserATNSimulator;dfa : DFA; D : DFAState) return DFAState is
      ...
      begin
            ...
            if debug then
               Text_IO.Put_Line ("adding new DFA state: " & D'Image);
            end if;
   ...
   procedure reportAttemptingFullContext (This : ParserATNSimulator; dfa : DFA; conflictingAlts : Optional_BitSet; configs : ATNConfigSet; startIndex : Integer; stopIndex : Integer) {
      if debug or else retry_debug then
            ...
            Text_IO.Put_Line ("reportAttemptingFullContext decision=" & dfa.decision & ":" & configs'Image & ", input=" & input'Image);
      end if;
   ...
   procedure reportContextSensitivity (This : ParserATNSimulator; dfa : DFA; prediction : Integer; configs : ATNConfigSet; startIndex : Integer; stopIndex : Integer) {
      if debug or else retry_debug then
            input : constant := getTextInInterval (startIndex, stopIndex);
            Text_IO.Put_Line ("reportContextSensitivity decision=" & dfa.decision & ":" & configs'Image & ", input=" & input'Image);
      end if;
      ...
      configs : ATNConfigSet);
   {
      if debug or else retry_debug then
            input : constant := getTextInInterval (startIndex, stopIndex);
            Text_IO.Put_Line ("reportAmbiguity " & ambigAlts'Image & ":" & configs'Image & ", input=" & input'Image);
      end if;
   ...
   -- private
   function getTextInInterval (This : ParserATNSimulator;startIndex : Integer; stopIndex : Integer) return UString is
begin
      interval : constant := Interval.of (startIndex, stopIndex);
      declare
      begin
            return parser.getTokenStream ()?.getText (interval), Default => "<unknown>";
      end if;
      exception
         when others =>
            return "<unknown>"
