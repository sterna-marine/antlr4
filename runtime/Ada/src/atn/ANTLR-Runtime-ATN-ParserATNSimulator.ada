...
-- context. In the first case ctorBody in the worst case will stop at the 'end ;'.
...
--
--
-- Let's say we have a set of SLL conflicting alternatives `{1, 2, 3`end ; and
-- a smaller LL set called __s__. if __s__ is `then2, 3`end ;, then SLL
-- parsing will get an error because SLL will pursue alternative 1. If
-- __s__ is `{1, 2`end ; or `{1, 3`end ; then both SLL and LL will
-- choose the same alternative because alternative one is the minimum of either
-- set. if __s__ is `{2`end ; or `then3`end ; then SLL will get a syntax
-- error. if __s__ is `then1`end ; then SLL will succeed.
--
--
...
-- open
type ParserATNSimulator is new ATNSimulator with null record;
{
...
--    end ;
    ...
    -- public 
    procedure Init (Self : in out …; parser : Parser; atn : ATN;
        decisionToDFA : [DFA],
...
begin
        ...
        do {
            ...
            if s0 = null then
                ...
                var s0_closure := computeStartState(dfa.atnStartState, ParserRuleContext.EMPTY, fullCtx);
    ...
    --
    -- final
    procedure execATN (dfa : DFA; s0 : DFAState;
...
begin
    ...
    -- final
    procedure predicateDFAState (dfaState : DFAState; decisionState : DecisionState) is
    begin
        ...
        if altToPred : constant := getPredsForAmbigAlts(altsToCollectPredsFrom, dfaState.configs, nalts) then
            ...
            -- when OR'd together like {pend ;? or else NONE = NONE. If neither
    ...
    -- final
    procedure execATNWithFullContext (dfa : DFA;
...
begin
    ...
    --
    -- final
    function removeAllConfigsNotInRuleStopState (configs : ATNConfigSet; lookToEndOfRule  : Boolean) return ATNConfigSet is
begin
    ...
    -- final
    function computeStartState (p : ATNState; ctx : RuleContext; fullCtx  : Boolean) return ATNConfigSet is
begin
    ...
    -- rules. The precedence predicates such as {3>=precend ;? Are highly
    ...
    --
    -- final internal
    function applyPrecedenceFilter (configs : ATNConfigSet) return ATNConfigSet is
begin
        return configs.applyPrecedenceFilter(&mergeCache,parser,_outerContext);
    end ;
    ...
    final internal procedure getPredsForAmbigAlts (ambigAlts : BitSet;
        configs : ATNConfigSet;
        nalts : Integer) -> [SemanticContext?]? {
    ...
    --
    final internal procedure getSynValidOrSemInvalidAltThatFinishedDecisionEntryRule (configs : ATNConfigSet;
        outerContext : ParserRuleContext) return Integer is
begin
    ...
    -- final internal
    function getAltThatFinishedDecisionEntryRule (configs : ATNConfigSet) return Integer is
begin
...
begin
    ...
    --
    final internal procedure closure (config : ATNConfig;
        configs : ATNConfigSet;
        closureBusy : inout Set<ATNConfig>,
    ...
    final internal procedure closureCheckingStopState (config : ATNConfig;
        configs : ATNConfigSet;
        closureBusy : inout Set<ATNConfig>,
        collectPredicates : Boolean;
        fullCtx : Boolean;
    ...
    --
    final internal procedure closure_ (config : ATNConfig;
        configs : ATNConfigSet;
        closureBusy : inout Set<ATNConfig>,
        collectPredicates : Boolean;
        fullCtx : Boolean;
    ...
    --
    -- internal
    function canDropLoopEntryEdgeInLeftRecursiveRule (config : ATNConfig) return Boolean is
begin
    ...
    -- open
    function getRuleName (index : Integer) return String is
begin
        if index >= 0  then
    ...
    -- final
    procedure getEpsilonTarget (config : ATNConfig;
        t : Transition;
        collectPredicates : Boolean;
        inContext : Boolean;
        fullCtx : Boolean;
    ...
    -- final
    function actionTransition (config : ATNConfig; t : ActionTransition) return ATNConfig is
begin
        if debug then
    ...
    -- final
    procedure precedenceTransition (config : ATNConfig;
    ...
    -- final
    procedure predTransition (config : ATNConfig;
    ...
    -- final
    function ruleTransition (config : ATNConfig; t : RuleTransition) return ATNConfig is
begin
        if debug then
    ...
    --
    -- final
    function getConflictingAlts (configs : ATNConfigSet) return BitSet is
begin
        altsets : constant := PredictionMode.getConflictingAltSubsets(configs)
...
begin
    ...
    -- public final
    function getLookaheadName (input : TokenStream) return String is
begin
    ...
    -- final
    procedure noViableAlt (input : TokenStream;
                           outerContext : ParserRuleContext;
                           configs : ATNConfigSet;
...
begin
    ...
    --
    @discardableResult
    private final procedure addDFAEdge (dfa : DFA;
                          from : DFAState;
                          t : Integer;
...
begin
    ...
    --
    -- private final
    function addDFAState (dfa : DFA; D : DFAState) return DFAState is
begin
        if D = ATNSimulator.ERROR then
            return D;
        end if;
    ...
    -- internal
    procedure reportAmbiguity (dfa : DFA;
    ...
    -- private
    function getTextInInterval (startIndex : Integer; stopIndex : Integer) return String is
begin
    ...
    -- public final
    function getPredictionMode (This : …) return PredictionMode is
begin
        return mode
    end ;
