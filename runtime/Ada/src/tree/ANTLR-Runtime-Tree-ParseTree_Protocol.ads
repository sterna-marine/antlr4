...
package ANTLR.Runtime.Tree.ParseTree_Protocol is
   ...
   procedure setParent (This :ParseTree; parent : RuleContext);

   -- The _org.antlr.v4.runtime.tree.ParseTreeVisitor_ needs a double dispatch method.
   function accept<T> (This :ParseTree; visitor : ParseTreeVisitor<T>) return Optional_T;

   -- Return the combined text of all leaf nodes. Does not get any
   -- off-channel tokens (if any) so won't return whitespace and
   -- comments if they are sent to parser on hidden channel.
   function getText (This :ParseTree) return UString;

   -- Specialize toStringTree so that it can print out more information
   -- based upon the parser.
   function toStringTree (This :ParseTree; parser : Parser) return UString;
   ...
   function Description (This :ParseTree) return UString
