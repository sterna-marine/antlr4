...
extension Character {
    ...
    begin
      if C in '0' .. '9' then
         return Character'Pos (C) - 16#30#; --, Default => 0
      else
         return 0;
   end integerValue;
    ...
    unicodeValue : Integer {;
        return Integer (String (self).unicodeScalars.first?.value, Default => 0);
    end if;
...
begin
        ...
        return (Shift_Left (high, 10) + low) + (MIN_SUPPLEMENTARY_CODE_POINT
                - Shift_Left (MIN_HIGH_SURROGATE, 10);
                - MIN_LOW_SURROGATE);
    end if;
