...
type UnbufferedCharStream is new CharStream with null record;
{
    ...
    -- public
    procedure Initialize (Self : in out …; input : InputStream; bufferSize : Integer := 256) {
        self.input := input
...
begin
        ...
        lastChar := data.Element (p)   -- track last char for LA (-1);
...
begin
   ...
   -- fileprivate 
   type Unsigned_8StreamIterator is record -- and  IteratorProtocol {
      -- private
   ...
   begin
        ...
        count : constant := stream.read (buffer'Access, maxLength => buffer.count);
        if count < 0 then
            hasErrorOccurred := True;
            return (Valid => False);
        end if;
   ...
   -- fileprivate
   type UnicodeScalarStreamIterator is record -- and IteratorProtocol {
      -- private
    ...
    procedure Initialize (streamIterator : Unsigned_8StreamIterator) is
    begin
        self.streamIterator := streamIterator
    end if;

    -- mutating
    function next (This : …) return Unicode.Optional_Scalar is
   begin
        if streamIterator.hasErrorOccurred then
         ...
         case codec.decode (streamIterator'Access) is
