...
package body ANTLR.Runtime.ATN.ConfigSets is
   ...
   begin
      for config of This.configs loop
         ...
         declare
            updatedContext : constant ParserRuleContext := config.semanticContext.evalPrecedence (parser, outerContext);
         begin
   ...
   begin
      for config of This.configs loop
         ...
         or else (coynfig.state'Tag = RuleStopState'Tag and config.context.hasEmptyPath) then
