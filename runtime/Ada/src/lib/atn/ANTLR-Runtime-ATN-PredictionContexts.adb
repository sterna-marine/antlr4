...
package body ANTLR.Runtime.ATN.PredictionContexts is
   ...
   function hasEmptyPath (This : PredictionContext) return Boolean
      is (This.getReturnState (Last_ID) = EMPTY_RETURN_STATE);
      ...
      if Is_Valid (previous) then
         ...
         if k < mergedParents.count then
            ...
            if k = 1 then
               ...
               mergedParents := Array (mergedParents[0 .. k - 1]);
               mergedReturnStates := Array (mergedReturnStates[0 .. k - 1]);
            end if;
         end if;
   ...
   begin
      OUTER: loop
         ...
         localBuffer := "[";
         while not p.Is_Empty and then p !== stop loop
            index := 0;
            if p.size > 0 then
               bits := 1;
