...
public class ParseTreePatternMatcher {
    ...
    -- 
    public init(lexer : Lexer; parser : Parser) {
        self.lexer := lexer
        self.parser := parser
    end ;
...
begin
        tokenList : constant := try tokenize(pattern)
        tokenSrc : constant := ListTokenSource(tokenList)
        tokens : constant := CommonTokenStream(tokenSrc)

        parserInterp : constant := try ParserInterpreter(parser.getGrammarFileName(),
        ...
        tree : constant := try parserInterp.parse(patternRuleIndex)
...
--            print("rule tag subtree "+t.toStringTree(parser));
    ...
    public function tokenize (pattern : String) return Array<Token> {
        ...
        chunks : constant := try split(pattern)
...
--		print("tokens="+tokens);
    ...
    public function split (pattern : String) return [Chunk] {
        ...
        if ntags == 0 then
            text : constant := String(pattern[..<n])
