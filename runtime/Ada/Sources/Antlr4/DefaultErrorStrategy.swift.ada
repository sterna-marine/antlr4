...
begin
        ...
        matchedSymbol : constant := try singleTokenDeletion(recognizer)
...
begin
        currentSymbolType : constant := try getTokenStream(recognizer).LA(1)
...
--		print("LT(2) set="+expectingAtLL2.toString(recognizer.getTokenNames()));
    ...
    open function singleTokenDeletion (recognizer : Parser) return Token? {
        nextTokenType : constant := try getTokenStream(recognizer).LA(2)
        expecting : constant := try getExpectedTokens(recognizer)
        if expecting.contains(nextTokenType) then
            ...
            matchedSymbol : constant := try recognizer.getCurrentToken()
...
begin
        currentSymbol : constant := try recognizer.getCurrentToken()
        expecting : constant := try getExpectedTokens(recognizer)
