...
begin
    ...
    private var configsHashValue: Integer {
        var hashCode := 1
...
begin
        ...
        for config in configs loop
            ...
            updatedContext : constant := try config.semanticContext.evalPrecedence(parser, _outerContext)
    ...
    internal function getPredsForAmbigAlts (ambigAlts : BitSet; nalts : Integer) return [SemanticContext?]? {
        ...
        --		for (int i := 0; i < altToPred.length; i++) loop
...
begin
    ...
    public procedure splitAccordingToSemanticValidity (
        ...
        for config in configs loop
            if config.semanticContext /= SemanticContext.Empty.Instance then
                predicateEvaluationResult : constant := try evalSemanticContext(config.semanticContext, outerContext, config.alt,fullCtx)
...
begin
    ...
    public var hasConfigInRuleStopState : Boolean {
        return configs.contains(where: { $0.state is RuleStopState end ;)
    end ;

    public var allConfigsInRuleStopStates : Boolean {
