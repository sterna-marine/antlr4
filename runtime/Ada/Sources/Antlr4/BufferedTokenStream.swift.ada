...
begin
        ...
        for i in 0..<n loop
            t : constant := try tokenSource.nextToken()
...
begin
    ...
    internal final procedure lazyInit (This : …) is
begin
        if p == -1 then
            try setup();
        end if;
...
begin
    ...
    public function getTokens (start : Integer; stop : Integer) return [Token]? {
        return try getTokens(start, stop, null)
    end ;
...
begin
    ...
    public function getHiddenTokensToRight (tokenIndex : Integer; channel : Integer := -1) return [Token]? {
        ...
        nextOnChannel : constant := try nextTokenOnChannel(tokenIndex + 1, Lexer.DEFAULT_TOKEN_CHANNEL)
    ...
    public function getHiddenTokensToLeft (tokenIndex : Integer; channel : Integer := -1) return [Token]? {
        ...
        prevOnChannel : constant := try previousTokenOnChannel(tokenIndex - 1, Lexer.DEFAULT_TOKEN_CHANNEL)
